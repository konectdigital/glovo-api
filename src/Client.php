...
use Konectdigital\GlovoApi\Exception;

class Client
{
	const API_BASE_URL_PRODUCTION = "https://api.glovoapp.com/b2b";
	const API_BASE_URL_SANDBOX    = "https://stageapi.glovoapp.com/b2b";
	...
	private static function build_request($request)
	{
		...
		$headers = array(
			...
			"Authorization: Basic " . base64_encode("{$request['auth']['apiKey']}:{$request['auth']['apiSecret']}"),
		);
		...
		curl_setopt($connect, CURLOPT_CAINFO, realpath(dirname(__FILE__) . '/../certs/ca-certificates.crt'));
		...
		$baseUri = ($request["sandbox"]) ? self::API_BASE_URL_SANDBOX : self::API_BASE_URL_PRODUCTION;
